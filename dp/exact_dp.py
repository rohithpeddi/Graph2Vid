...
def drop_dtw(
    zx_costs, drop_costs, exclusive=True, contiguous=True, one_to_one=False, return_labels=False, return_cost=False
):
...
def double_drop_dtw(pairwise_zx_costs, x_drop_costs, z_drop_costs, contiguous=True):
    ...
    for zi in range(1, K + 1):
        for xi in range(1, N + 1):
            ...
            neigh_coords___ = np.array(
                left_neg_neigh_coords + top_neg_neigh_coords
...
def otam(dist, exclusive=False):
    ...
    while (i >= 0) and (j >= 0):
        neighbors = (
            (otam[i - 1, j - 1], otam[i, j - 1]) if exclusive else (otam[i - 1, j - 1], otam[i, j - 1], otam[i - 1, j])
        )
